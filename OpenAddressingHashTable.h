...
#ifndef HASHTABLE_OPENADDRESSINGHASHTABLE_H
#define HASHTABLE_OPENADDRESSINGHASHTABLE_H
...
// OpenAddressingHashTable is an abstract base class for an open addressing hash table
template <typename K, typename V>
...
public:
    ...
    bool Insert(const K& key, const V& value) override {
        ...
        for (int i = 0; i < table.size(); i++) {
            ...
            if (table[bucketIndex] == &OpenAddressingBucket<K,V>::EMPTY_SINCE_START) {
                break;
            }

            if (table[bucketIndex] != &OpenAddressingBucket<K,V>::EMPTY_AFTER_REMOVAL) {
...
#endif //HASHTABLE_OPENADDRESSINGHASHTABLE_H
